...
procedure TForm1.Button1Click(Sender: TObject);        //Построение графика 1 функции
var
...
procedure TForm1.Button2Click(Sender: TObject);   //Очистка поля
begin
  Series1.Clear;
  Series2.Clear;
  chart1.UndoZoom;
...
procedure TForm1.Button3Click(Sender: TObject);   //Выбор цвета  1 функции
begin
  colordialog1.Execute;
end;

procedure TForm1.Button4Click(Sender: TObject);    //Выбор цвета 2 функции
begin
  colordialog2.Execute;
end;

procedure TForm1.Button5Click(Sender: TObject);    //Построение графика 2 функции
var
  a, b, c, d, range1, range2: double;
  Graph1: TCubic;
  Graph2: TCos;
...
begin
  ...
  begin
    ...
    if combobox2.ItemIndex = 0 then    //Кубическое уравнение
    begin
      Graph1:= TCubic.Create(a, b, c, d);
      Graph1.Draw(range1, range2, Series2, ColorDialog2.Color);
    end;

    if combobox2.ItemIndex = 1 then    //Косинус
    begin
      Graph2 := TCos.Create(a, b, c, d);
      Graph2.Draw(range1, range2, Series2, ColorDialog2.Color);
    end;

    if combobox2.ItemIndex = 2 then    //Синус
    begin
      Graph3 := Tsin.Create(a, b, c, d);
...
procedure TForm1.Edit1KeyPress(Sender: TObject; var Key: Char);  //Ограничение на ввод
begin
...
procedure TForm1.N1Click(Sender: TObject);   //Сохранение графика
begin
Chart1.SaveToBitmapFile(FormatDateTime('График_' + 'dd.mm.yyyy_hh.mm', now)+'.bmp');
showmessage('График сохранен!');
end;

procedure TForm1.N2Click(Sender: TObject);   //О программе
begin
 form3.ShowModal;
end;

procedure TForm1.RadioGroup1Click(Sender: TObject);  //Переключение шкалы
begin
...
if radiogroup1.ItemIndex=1 then
 ...
 on AxisException do
  begin
   radiogroup1.ItemIndex:=0;
   showmessage('Построение в логарифмическом масштабе невозможно!' + #13#10 +
   'Измените диапазон!');
  end;
 end;
